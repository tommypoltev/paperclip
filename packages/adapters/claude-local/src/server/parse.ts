...
function buildClaudeTransientHaystack(input: {
  ...
  return buildClaudeProviderErrorHaystack({
    ...input,
    parsedResultText: resultText,
...
export function extractClaudeRetryNotBefore(
  ...
  const parsed = input.parsed ?? null;
  return extractClaudeProviderQuotaRetryNotBefore(
    {
      ...input,
      parsedResultText: parsed ? asString(parsed.result, "") : "",
      parsedErrorMessages: parsed ? extractClaudeErrorMessages(parsed) : [],
    },
    now,
  );
}
...
export function isClaudeProviderQuotaError(input: {
  ...
  return isClaudeProviderQuotaText({
    ...input,
    parsedResultText: parsed ? asString(parsed.result, "") : "",
    parsedErrorMessages: parsed ? extractClaudeErrorMessages(parsed) : [],
  });
